...
search_url = "https://maps.googleapis.com/maps/api/place/textsearch/json"  #API url for text search taken from API google developers guide object type is json
details_url = "https://maps.googleapis.com/maps/api/place/details/json"   # API url for place details taken from API google developers guide
...
def results(query):
    ...
    search_req = requests.get(search_url, params=search_payload) # we are sending it to search url and params value is the value in search payload . This will give output as response
    search_json = search_req.json()  # This will convert the above returned value in the Json format.
